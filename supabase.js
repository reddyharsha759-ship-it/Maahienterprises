(function () {
  ...
  // Check if we are running in automated test mode
  var isTestMode = localStorage.getItem("maahi_test_mode") === "true";
  if (isTestMode) {
    window.maahiSupabase = {
      reinit: function () { return true; },
      getClient: function () { return { isMock: true }; },
      isConnected: function () { return true; },
      testConnection: function () { return Promise.resolve({ success: true, message: "Connection successful! (Mocked)" }); },
      syncLocalCatalog: function () { return Promise.resolve(true); },
      fetchCatalog: function () {
        try {
          var raw = localStorage.getItem("maahi_catalog_v1");
      ...
      saveProduct: function (id, product) {
        try {
          var raw = localStorage.getItem("maahi_catalog_v1");
          var catalog = raw ? JSON.parse(raw) : {};
          catalog[id] = product;
          localStorage.setItem("maahi_catalog_v1", JSON.stringify(catalog));
        } catch(e) {}
      ...
      fetchOrders: function () {
        try {
          var raw = localStorage.getItem("glc_orders_v1");
      ...
      saveOrder: function (order) {
        try {
          ...
          list.unshift(order);
          localStorage.setItem("glc_orders_v1", JSON.stringify(list));
        } catch(e) {}
        return Promise.resolve(true);
      },
      ...
      signInWithGoogle: function () { return Promise.resolve(true); },
